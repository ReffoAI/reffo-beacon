...
router.post('/', (req: Request, res: Response) => {
  const items = new ItemQueries();
  const { name, description, category, subcategory, image, sku } = req.body;
  ...
  const item = items.create({ name, description, category, subcategory, image, sku }, beaconId);
...
router.patch('/:id', (req: Request, res: Response) => {
  ...
  const updated = items.update(String(req.params.id), { name, description, category, subcategory, image, sku });
